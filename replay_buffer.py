...
class ReplayBuffer(object):
    def __init__(self, buffer_size, DIRECTORY, max_len_trajectory, last_saved_epi):
        ...
        # self.buffer = deque()
        self.buffer_dict = OrderedDict()  # one buffer_dict have a trajectory (a series of Marko Process)
        ...
        self.next_episode = 0
        # earliest-episode to feed into the buffer (aim to judge if this element belong to this episode)
        self.last_saved_epi = last_saved_epi
        self.index = 0  # current time-step in current episode
        self.counts_per_epi = np.array([])  # the counter per episode
        ...
        self.rewards_per_epi = np.array([])  # reward per episode
    ...
    def get_batch(self, batch_size, max_trace_length, time_step):
        ...
        if (trace_length < max_trace_length):
            # Just in case when the trace_length of the model is too long but episodes end earlier
            length_mode = 'short'
        else:
            length_mode = 'long'
        ...
        for sampled_epi in sampled_epi_list:
            try:
                if length_mode == 'long':
                    ...
                    # index_last = np.random.randint(
                    #     low=trace_length-1,
                    #     high=self.counts_per_epi[sampled_epi-self.earliest_episode],
                    #     size=1, dtype=int)[0]
    ...
    def add(self, state, action, reward, new_state, done, epi, time_step):
        ...
        if self.initial_add is True:  # add the first element
            self.dim = np.zeros(shape=(len(experience)), dtype=object)  # init the dim = 8
        ...
        if self.next_episode == epi:  # First time-step for each epi
            self.buffer_dict[epi] = np.zeros(shape=(self.len_trajectory, self.categories), dtype=object)
            # 8 = len(experience), self.len_trajectory > 1600 + TRACE_LENGTH = env.spec.timestep_limit + TRACE_LENGTH
            self.index = 0
            try:
            ...
            self.rewards_per_epi = np.append(self.rewards_per_epi, experience[2])  # get her this reward and this state
            ...
            self.rewards_per_epi[-1] = \
                self.rewards_per_epi[-1]*float(self.index)/(float(self.index)+1.) \
                    + experience[4] * 1./(float(self.index)+1.)

        if done:  # if done , the follow of this episode trajectory will be delete
            deletion = np.arange(self.index+1, self.len_trajectory)
            self.buffer_dict[epi] = np.delete(self.buffer_dict[epi], deletion, axis=0)
            self.reward_series_discounted = 0
            reward_series_discounted = self.buffer_dict[epi][:, 4]  # calculate the reward of this episode in total
...
if __name__ == '__main__':
    replay_buffer = ReplayBuffer(buffer_size=int(1e6),\
                                 ...
                                 max_len_trajectory=2001, last_saved_epi=-1)
    for i in range(100):
        replay_buffer.add(state=np.random.randn(24), action=np.random.randn(4), reward=np.random.randn(1),
                          new_state=np.random.randn(24), done=False, epi=0,
                          time_step=i)
    replay_buffer.add(state=np.random.randn(24), action=np.random.randn(4), reward=np.random.randn(1),
                      new_state=np.random.randn(24), done=True, epi=0,
                      time_step=100)
    for i in range(100):
        replay_buffer.add(state=np.random.randn(24), action=np.random.randn(4), reward=np.random.randn(1),
                          new_state=np.random.randn(24), done=False, epi=1,
                          time_step=i)
    replay_buffer.add(state=np.random.randn(24), action=np.random.randn(4), reward=np.random.randn(1),
                      new_state=np.random.randn(24), done=True, epi=1,
                      time_step=100)
    a = replay_buffer.get_batch(batch_size=12, max_trace_length=12, time_step=4)
    print(a[0].shape)
